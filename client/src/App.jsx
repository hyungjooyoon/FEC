...
class App extends React.Component {
  ...
  getRating(id) {
    return axios.get(`/api/reviews/meta/${id}`)
      .then((response) => {
        let count = 0;
        let total = 0;
        for (const rating in response.data.ratings) {
    ...
    return axios.get(`/api/products/${id}/related`)
      ...
      .then(() => axios.all(promises)
        .then(axios.spread((...responses) => {
          ...
          return {
            newIds, newRatings, newReviews, newProducts, newStyles, relatedProducts,
          };
        }))
        .catch((err) => console.error(err)));
  }
  ...
  changeProductId(id) {
    const ind = this.state.productIds.indexOf(id);
    let product;
    let styles;
    let rating;
    let review;
    ...
    if (ind !== -1) {
      ...
      return this.getProduct(id)
        ...
        .then(() => this.getStyles(id))
        .then((response) => {
          styles = response.data;
          newStyles.push(styles);
        })
        .then(() => this.getRating(id))
        .then((response) => {
        ...
        .then((data) => {
          ...
          const relatedData = products.filter((product) => data.relatedProducts.includes(String(product.id)));
          const allStyles = this.state.stylesData.concat(newStyles);
          const allRatings = this.state.ratings.concat(newRatings);
          const allIds = this.state.productIds.concat(newIds);
          const found = [];
